...
def load_vgg(sess, vgg_path):
    ...
    return layer_input, keep, layer_3, layer_4, layer_7
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    layer4_1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='SAME',
                               kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # combining layer 4 with the upsample of layer 7
    add_74 = tf.add(layer7_us, layer4_1x1)
    # upsample
    add_74_us = tf.layers.conv2d_transpose(add_74, num_classes, 4, 2, 'SAME',
                               kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    layer3_1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='SAME',
                               kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # combining layer 3 with the upsample of previous layer
    add_743 = tf.add(add_74_us, layer3_1x1)
    # upsample
    out = tf.layers.conv2d_transpose(add_743, num_classes, 16, 8, 'SAME',
                               kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        # Append avarage loss in epoch
        loss_history.append(epoch_loss/batches_cnt)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        loss_history = train_nn(sess, EPOCHS, BATCH_SIZE, get_batches_fn, train_op, cross_entropy_loss, input_image, correct_label,
...
if __name__ == '__main__':
    run()
    #run_original()
    #run_(graph_name='frozen_graph.pb')
    #run_(graph_name='optimized_graph.pb')
    #run_(graph_name='eightbit_graph.pb')
